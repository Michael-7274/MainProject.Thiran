...
export default function AssignProducts({ logout }) {
  ...
  //only intializes once but usEffect is executed every time the component is mounted
  ...
  const filterProducts = () => {
    //causes rerender error if setItemCount() is used inside filterProducts()
    if(fetchError)
    {
      return (
        <>
          <h1>Can't fetch products at the moment, sorry for the inconvenience</h1>
        </>
      );
    }
